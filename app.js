...
async function readImageWithAI(){
  const f=$("#imageInput").files?.[0];
  if(!f){status("#ocrStatus","Choose photo first","error");return}
  status("#ocrStatus","AI Vision scan started. Sending photo to local secure backend...");
  try{
    const fd=new FormData();
    fd.append('photo',f,f.name||'inspection-photo.png');
    const res=await fetch('/api/vision-ocr',{method:'POST',body:fd});
    if(!res.ok){
      let msg='AI backend not available.';
      try{const j=await res.json(); msg=j.error||msg;}catch(e){msg=await res.text()||msg;}
      throw new Error(msg);
    }
    ...
    status("#ocrStatus",`AI Vision completed. Auto-filled ${kcount}/15 KPI scores plus detected written fields. Review once, then update Excel.`,kcount?'success':'error');
  }catch(e){
    status("#ocrStatus","AI Vision scan failed: "+e.message+". Start the local AI server using start_ai_server.bat, add your OPENAI_API_KEY in .env, or use Option A2 browser OCR backup.","error");
  }
}
