...
function validateKpiScores(found, sourceText){
  // Strict safety gate: prevents wrong values being pushed into Excel.
  // Browser OCR can confuse row/date/package/cluster numbers with KPI values.
  const cleaned={};
  let count=0;
  KPI.forEach(k=>{
    const v=scoreToken(found && found[k[1]]);
    if(v){ cleaned[k[1]]=v; count++; }
  });
  const txt=String(sourceText||'');
  const hasRealInspectionContext=/inspection|cluster|package|contractor|work at height|edge protection|scaffold|ptw|excavation|lifting|traffic|housekeeping/i.test(txt);
  if(count<10){
    return {scores:{}, count, accepted:false, reason:'Less than 10 KPI values were confidently detected, so scores were not auto-applied.'};
  }
  if(!hasRealInspectionContext && count<15){
    return {scores:{}, count, accepted:false, reason:'The image does not look like an inspection/KPI sheet, so scores were not auto-applied.'};
  }
  return {scores:cleaned, count, accepted:true, reason:'KPI values passed the safety check.'};
}

function applyDetectedFields(text,found){
  const check=validateKpiScores(found,text);
  found=check.scores;
  let filled=[];
  ...
  return {filled,kcount,ocrAccepted:check.accepted,ocrReason:check.reason,rawKpiCount:check.count};
}
...
async function readImage(){
  ...
  try{
    ...
    const msg=`Smart scan completed. Auto-filled ${result.kcount}/15 KPI scores and ${result.filled.length} other fields: ${result.filled.join(', ')||'none'}. ${result.ocrReason} Review highlighted fields before updating Excel.`;
    status("#ocrStatus",msg,(totalFields&&result.ocrAccepted)?"success":(totalFields?"":"error"));
  }catch(e){
