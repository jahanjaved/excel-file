...
function setMasterStatus(msg,tone="normal"){ const box=$("#masterStatus"); box.textContent=msg; box.style.background=tone==="error"?"#fff3f3":tone==="success"?"#f3fff7":"#fff"; }
...
function extractScores(text){
  ...
  KPI_WEIGHTS.forEach(k=>{ if(!scores[k[0]]) scores[k[0]]=""; });
  return scores;
}
...
function allScores(){ const o={}; document.querySelectorAll(".score-input").forEach(x=>{ o[x.dataset.key]=x.value.trim(); }); return o; }
...
function updateDerived(){
  ...
  if(CLUSTERS[cluster]){ $("#package").value=String(CLUSTERS[cluster].pkg); $("#contractor").value=CLUSTERS[cluster].contractor; }
  recalc(false);
}
function renderKpis(){
  const grid=$("#kpiGrid");
  grid.innerHTML=KPI_WEIGHTS.map((k,i)=>`<div class="kpi-card"><h3>${i+1}. ${k[2]}</h3><input class="score-input" data-key="${k[0]}" placeholder="Score from photo or blank" /></div>`).join("");
...
function recalc(userEdited=false){
  ...
  $("#summaryBox").innerHTML=count?`<strong>Readable KPI scores:</strong> ${count}/15 | <strong>Total:</strong> ${total} | <strong>Raw %:</strong> ${(raw*100).toFixed(1)}% | <strong>Weighted %:</strong> ${(wp*100).toFixed(1)}% | <strong>Critical Red Flag:</strong> ${critical?"YES":"NO"} | <strong>Band:</strong> ${band}`:"No readable KPI scores yet. Blank fields will remain empty in the inserted row.";
}
...
async function readImage(){
  ...
  try{
    ...
    for(let i=0;i<vars.length;i++){
      ...
      const text=result.data.text||""; const sc=Object.values(extractScores(text)).filter(v=>v!=="").length+(extractDate(text)?3:0)+(extractClusterPackage(text).cluster?3:0);
    ...
    const villa=extractVilla(ocr); $("#areaText").value=villa;
    const scores=extractScores(ocr); document.querySelectorAll(".score-input").forEach(x=>{ x.value=scores[x.dataset.key]||""; });
    ...
    if($("#cluster").value && CLUSTERS[$("#cluster").value]) $("#contractor").value=CLUSTERS[$("#cluster").value].contractor;
    ...
    const readable=Object.values(scores).filter(v=>v!=="").length;
    setOcrStatus(`Photo reading complete. ${readable}/15 KPI scores detected. Blank/unreadable KPI fields are left empty for manual entry.`, readable?"success":"error");
    setStatus("Review extracted fields. Generated Excel will insert one styled row into the master file.","success");
...
function buildMappedValues(){
  ...
  const map={};
  const set=(h,v)=>{ if(v!=="" && v!==null && v!==undefined) map[h]=v; };
  set("Inspection_ID", d&&cluster?`${d.replaceAll("-","")}-${cluster.replace(/\s+/g,"")}-01`:"");
  set("Inspection_Date", d?isoDateSerial(d):"");
  set("Week_No", weekNo(d));
  set("Month", d?d.slice(0,7):"");
  set("Package", pkg?Number(pkg):"");
  set("Cluster", cluster);
  set("Package_Cluster_Check", cluster&&pkg&&CLUSTERS[cluster]&&String(CLUSTERS[cluster].pkg)===String(pkg)?"OK":(cluster||pkg?"CHECK":""));
  set("Contractor", $("#contractor").value.trim() || CLUSTERS[cluster]?.contractor || "");
  set("Your_Name", $("#yourName").value.trim());
  set("Your_Role", $("#yourRole").value.trim());
  set("Area_or_Villa", $("#areaText").value.trim());
  set("Main_High_Risk_Activity", $("#mainActivity").value.trim());
  set("Stop_Work", $("#stopWork").value);
  KPI_WEIGHTS.forEach(k=>{ const val=scores[k[0]]; if(val!=="") set(k[0], scoreVal(val) ?? val); });
  set("Total_Score", count?total:"");
  set("Raw_Score_%", raw);
  set("Weighted_Score_%", wp);
  set("Critical_Red_Flag", count?(critical?"YES":"NO"):"");
  set("Award_Eligible", award);
  set("Rating_Band", band);
  set("Repeat_Issue_Seen", $("#repeatIssue").value);
  set("Likely_Root_Cause", $("#rootCause").value.trim());
  set("Top_3_Gaps_Observed", $("#topGaps").value.trim());
  set("Immediate_Action_Taken", $("#immediateAction").value.trim());
  set("Preventive_Action_Required", $("#preventiveAction").value.trim());
  set("Positive_Observations", $("#positiveObservations").value.trim());
  set("Action_Owner", $("#actionOwner").value.trim());
  set("Target_Closeout_Date", $("#targetCloseoutDate").value?isoDateSerial($("#targetCloseoutDate").value):"");
  set("Evidence_Link_or_Photo", $("#evidence").value.trim());
  return map;
}
function getRangeEnd(ws){
  const ref=ws["!ref"] || "A1:A1";
  return XLSX.utils.decode_range(ref);
}
function rowHasValue(ws,row,range){
  for(let c=range.s.c;c<=range.e.c;c++){
    const a=XLSX.utils.encode_cell({r:row,c});
    const cell=ws[a];
    if(cell && cell.v!==undefined && cell.v!==null && String(cell.v)!=="") return true;
  }
  return false;
}
function findHeaderRow(ws,range){
  for(let r=range.s.r;r<=Math.min(range.e.r,30);r++){
    ...
    for(let c=range.s.c;c<=range.e.c;c++){
      ...
      vals.push(String(cell?.v||"").trim());
    }
    const score=HEADERS.filter(h=>h && vals.includes(h)).length;
    if(score>=10) return r;
  }
  return 2;
}
function headerMap(ws,headerRow,range){
  const m={};
  for(let c=range.s.c;c<=range.e.c;c++){
    const v=String(ws[XLSX.utils.encode_cell({r:headerRow,c})]?.v||"").trim();
    if(v) m[v]=c;
  }
  return m;
}
function cloneCellStyle(src,target){
  if(!src) return;
  if(src.s) target.s=JSON.parse(JSON.stringify(src.s));
  if(src.z) target.z=src.z;
  if(src.w) delete target.w;
}
function shiftFormula(f,fromRow,toRow){
  if(!f) return f;
  const from=String(fromRow+1), to=String(toRow+1);
  return String(f).replace(new RegExp(`(?<![A-Za-z0-9_])([A-Z]{1,3})\\$?${from}(?![0-9])`,"g"),(m,col)=>m.replace(from,to));
}
function cloneSourceRow(ws,sourceRow,targetRow,range){
  for(let c=range.s.c;c<=range.e.c;c++){
    const src=ws[XLSX.utils.encode_cell({r:sourceRow,c})];
    const addr=XLSX.utils.encode_cell({r:targetRow,c});
    const cell={t:"s",v:""};
    if(src){
      cloneCellStyle(src,cell);
      if(src.f){ cell.f=shiftFormula(src.f,sourceRow,targetRow); cell.t=src.t||"n"; }
      else { cell.t=src.t||"s"; cell.v=""; }
    }
    ws[addr]=cell;
  }
  if(ws["!rows"] && ws["!rows"][sourceRow]){
    ws["!rows"][targetRow]=JSON.parse(JSON.stringify(ws["!rows"][sourceRow]));
  }
}
function setCellValue(ws,row,col,value,sourceCell){
  const addr=XLSX.utils.encode_cell({r:row,c:col});
  const cell=ws[addr] || {};
  cloneCellStyle(sourceCell,cell);
...
function updateRef(ws,range,targetRow){
  if(targetRow>range.e.r) range.e.r=targetRow;
  ws["!ref"]=XLSX.utils.encode_range(range);
}
async function loadMasterWorkbook(){
  if(selectedMasterFile){
    const buf=await selectedMasterFile.arrayBuffer();
    return XLSX.read(buf,{type:"array",cellStyles:true,cellDates:false});
  }
  const res=await fetch("template.xlsx");
  if(!res.ok) throw new Error("template.xlsx not found. Please upload the master Excel file.");
  const buf=await res.arrayBuffer();
  return XLSX.read(buf,{type:"array",cellStyles:true,cellDates:false});
}
async function generateWorkbook(){
  try{
    setStatus("Preparing master file and copying design from existing row...","normal");
    const wb=await loadMasterWorkbook();
    const sheetName=wb.SheetNames.find(n=>/weekly/i.test(n)) || wb.SheetNames[0];
    const ws=wb.Sheets[sheetName];
    let range=getRangeEnd(ws);
    const hRow=findHeaderRow(ws,range);
    const hMap=headerMap(ws,hRow,range);
    let last=hRow;
    for(let r=hRow+1;r<=range.e.r;r++){ if(rowHasValue(ws,r,range)) last=r; }
    const newRow=last+1;
    cloneSourceRow(ws,last,newRow,range);
    const data=buildMappedValues();
    Object.entries(data).forEach(([h,v])=>{
      if(v==="" || v===null || v===undefined) return;
      const c=hMap[h];
      if(c===undefined) return;
      const sourceCell=ws[XLSX.utils.encode_cell({r:last,c})];
      setCellValue(ws,newRow,c,v,sourceCell);
    });
    updateRef(ws,range,newRow);
    if(!ws["!rows"]) ws["!rows"]=[];
    if(ws["!rows"][last] && !ws["!rows"][newRow]) ws["!rows"][newRow]=JSON.parse(JSON.stringify(ws["!rows"][last]));
    const outName=`Aldar_Weekly_Master_Updated_${$("#inspectionDate").value||"new_row"}.xlsx`;
    XLSX.writeFile(wb,outName,{bookType:"xlsx",cellStyles:true});
    setStatus(`Done. New row inserted in ${sheetName} at Excel row ${newRow+1}. Style, size and design copied from previous record. Only available mapped fields were filled.`,"success");
  }catch(e){
    console.error(e);
    setStatus("Excel generation failed: "+e.message,"error");
  }
}
document.addEventListener("DOMContentLoaded",()=>{
  renderKpis();
  $("#actionOwner").value="Project Manager and Construction Manager";
  $("#evidence").value="A site reference provided";
  $("#repeatIssue").value="YES";
  $("#inspectionDate").addEventListener("change",updateDerived);
  $("#cluster").addEventListener("input",updateDerived);
  $("#package").addEventListener("input",()=>{ const p=$("#package").value.trim(); if(p==="3") $("#contractor").value="INNOVO"; if(p==="2"||p==="4") $("#contractor").value="TAJV"; });
  $("#readBtn").addEventListener("click",readImage);
  $("#generateBtn").addEventListener("click",generateWorkbook);
  $("#masterInput").addEventListener("change",e=>{ selectedMasterFile=e.target.files?.[0]||null; setMasterStatus(selectedMasterFile?`Selected: ${selectedMasterFile.name}`:"Master file not selected yet.", selectedMasterFile?"success":"normal"); });
