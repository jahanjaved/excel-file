...
async function extractChecklistScoreColumn(file){
  // Dedicated ALDAR printed checklist reader.
  // It crops only the SCORE column and OCRs digits/decimals in vertical KPI order.
  try{
    const img=await new Promise((resolve,reject)=>{const im=new Image(); im.onload=()=>resolve(im); im.onerror=reject; im.src=URL.createObjectURL(file)});
    const attempts=[
      {name:'standard score column',x1:.505,x2:.615,y1:.205,y2:.655},
      {name:'wide score column',x1:.485,x2:.640,y1:.190,y2:.675},
      {name:'right-shift score column',x1:.525,x2:.655,y1:.200,y2:.670}
    ];
    let best=[];
    for(const a of attempts){
      const sx=Math.max(0,Math.round(img.width*a.x1));
      const sy=Math.max(0,Math.round(img.height*a.y1));
      const sw=Math.min(img.width-sx,Math.round(img.width*(a.x2-a.x1)));
      const sh=Math.min(img.height-sy,Math.round(img.height*(a.y2-a.y1)));
      const scale=7;
      const canvas=document.createElement('canvas'); canvas.width=sw*scale; canvas.height=sh*scale;
      const ctx=canvas.getContext('2d',{willReadFrequently:true});
      ctx.imageSmoothingEnabled=false; ctx.drawImage(img,sx,sy,sw,sh,0,0,canvas.width,canvas.height);
      const data=ctx.getImageData(0,0,canvas.width,canvas.height);
      for(let i=0;i<data.data.length;i+=4){
        const r=data.data[i],g=data.data[i+1],b=data.data[i+2];
        let y=0.299*r+0.587*g+0.114*b;
        // Blue pen becomes dark, background becomes white.
        y=y<205?0:255;
        data.data[i]=data.data[i+1]=data.data[i+2]=y;
      }
      ctx.putImageData(data,0,0);
      ...
      const r=await Tesseract.recognize(blob,'eng',{
        tessedit_char_whitelist:'012345., ',
        tessedit_pageseg_mode:6,
        preserve_interword_spaces:'1'
      });
      const txt=(r.data.text||'').replace(/,/g,'.');
      const nums=(txt.match(/\b[0-5](?:\.5)?\b/g)||[]).map(scoreToken).filter(Boolean);
      if(nums.length>best.length) best=nums;
      if(best.length>=15) break;
    }
    if(best.length>=10){
      const chosen=best.length>15?best.slice(0,15):best;
      const out={};
      KPI.forEach((k,i)=>{ if(chosen[i]) out[k[1]]=chosen[i]; });
      return out;
    }
  }catch(e){ console.warn('Checklist score-column OCR failed',e); }
  return {};
}
...
async function readImage(){
  ...
  status("#ocrStatus","Smart scan started. First reading the dedicated ALDAR SCORE column...");
  try{
...
function getOpenAiKey(){
  ...
  const fromBox=keyInput?keyInput.value.trim():'';
  if(fromBox){ localStorage.setItem('ALDAR_OPENAI_KEY',fromBox); return fromBox; }
  return localStorage.getItem('ALDAR_OPENAI_KEY')||'';
}
function getAiModel(){
  return ($('#openaiModel')?.value||'gpt-4.1-mini').trim()||'gpt-4.1-mini';
}
async function readImageWithAI(){
  const f=$('#imageInput').files?.[0];
  if(!f){status('#ocrStatus','Choose photo first','error');return}
  const apiKey=getOpenAiKey();
  if(!apiKey){status('#ocrStatus','Please paste your OpenAI API key in the AI key box first. Without key, GitHub Pages cannot call AI Vision.','error');return}
  status('#ocrStatus','AI Vision scan started from website. Reading handwritten values with OpenAI Vision...');
  try{
...
Return this JSON shape exactly:
{
  ...
  "lowest3":[]
    ...
    const data=JSON.parse(cleanAiJsonText(text));
    applyAiVisionResult(data);
    $('#ocrText').value=JSON.stringify(data,null,2);
    const kcount=Object.values(data.scores||{}).filter(Boolean).length;
    status('#ocrStatus',`AI Vision completed from website. Auto-filled ${kcount}/15 KPI scores plus detected fields. Review once, then update Excel.`,kcount?'success':'error');
  }catch(e){
    status('#ocrStatus','AI Vision failed: '+e.message+'. Check API key, internet connection, and model name. Browser OCR backup is still available.','error');
  }
}
function setIfValue(selector,value){if(value!==undefined&&value!==null&&String(value).trim()!==""){$(selector).value=String(value).trim(); return true} return false}
function applyAiVisionResult(data){
  data=data||{};
  setIfValue('#inspectionDate',data.inspectionDate||data.date);
  setIfValue('#cluster',data.cluster);
  setIfValue('#package',data.package);
  setIfValue('#contractor',data.contractor);
  setIfValue('#yourName',data.inspectorName||data.inspector_name);
  setIfValue('#yourRole',data.role||data.yourRole);
  setIfValue('#stopWork',String(data.stopWork||data.stop_work||'').toUpperCase());
  setIfValue('#areaText',data.area||data.villa||data.areaText);
  setIfValue('#mainActivity',data.mainActivity||data.main_high_risk_activity);
  setIfValue('#repeatIssue',String(data.repeatIssue||data.repeat_issue||'').toUpperCase());
  setIfValue('#actionOwner',data.actionOwner||data.action_owner);
  setIfValue('#evidence',data.evidence);
  const found=data.scores||{};
  document.querySelectorAll('.score-input').forEach(x=>{
    let v=found[x.dataset.key];
    if(v===undefined){
      const label=(KPI.find(k=>k[1]===x.dataset.key)||[])[2];
      v=found[label]||found[String(label||'').replace(/\s+/g,'_')];
    }
    v=scoreToken(v);
    if(v) x.value=v;
  });
  if(Array.isArray(data.lowest3)||Array.isArray(data.lowest_3)){
    const arr=data.lowest3||data.lowest_3;
    if(!$('#topGaps').value) $('#topGaps').value=arr.map((x,i)=>`${i+1}. ${x}`).join('\n');
  }
  updateDerived(); calc(true);
}

document.addEventListener("DOMContentLoaded",()=>{renderKpis(); const k=$('#openaiKey'); if(k && localStorage.getItem('ALDAR_OPENAI_KEY')) k.value=localStorage.getItem('ALDAR_OPENAI_KEY'); $("#imageInput").onchange=e=>{const f=e.target.files?.[0]; if(!f)return; const r=new FileReader(); r.onload=ev=>{$("#preview").src=ev.target.result;$("#preview").style.display="block";$("#ph").style.display="none";status("#ocrStatus","Photo loaded. Starting AI Vision directly from website...");setTimeout(readImageWithAI,250)}; r.readAsDataURL(f)}; $("#excelInput").onchange=e=>{excelFile=e.target.files?.[0]||null; status("#excelStatus",excelFile?`Selected original file: ${excelFile.name}`:"Included original.xlsx ready.",excelFile?"success":"")}; $("#readBtn").onclick=readImage; const aiBtn=$("#aiReadBtn"); if(aiBtn) aiBtn.onclick=readImageWithAI; $("#updateBtn").onclick=updateExcel; ["inspectionDate","cluster","package","contractor","yourName","yourRole","areaText","stopWork","repeatIssue","actionOwner","evidence"].forEach(id=>{const el=$("#"+id); if(el){el.addEventListener("input",updateDerived);el.addEventListener("change",updateDerived)}}); updateDerived(); calc(false)});
