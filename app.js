...
async function readImage(){
  ...
  status("#ocrStatus","Option A running: reading photo with enhanced OCR...");
  try{
    const pf=await preprocessForOcr(f).catch(()=>f);
    const r=await Tesseract.recognize(pf||f,"eng",{
      tessedit_pageseg_mode: 6,
      logger:m=>{if(m.status==="recognizing text")status("#ocrStatus",`OCR ${Math.round(m.progress*100)}%`)}
    });
    const t=r.data.text||"";
    $("#ocrText").value=t;
    ...
    if(cp.cluster&&CLUSTERS[cp.cluster])$("#contractor").value=CLUSTERS[cp.cluster].contractor;
    const m=String(t).match(/\b(?:villa|vila|vl)\s*[:\-]?\s*(\d{1,4}[A-Z]?)\b/i); if(m)$("#areaText").value=`Villa ${m[1]}`;

    const found=extractScores(t);
    let filled=0;
    document.querySelectorAll(".score-input").forEach(x=>{ if(found[x.dataset.key]){x.value=found[x.dataset.key]; filled++;} });
    updateDerived(); calc(true);
    const msg=filled>=10
      ? `Option A completed: ${filled}/15 KPI scores auto-filled. Option B: review and correct any value before Update Excel.`
      : `Option A found only ${filled}/15 KPI scores. Option B: use OCR Text/manual KPI fields to correct before Update Excel.`;
    status("#ocrStatus",msg,filled>=10?"success":"error");
  }catch(e){status("#ocrStatus","OCR failed: "+e.message+". Option B: enter values manually and update Excel.","error")}
}
