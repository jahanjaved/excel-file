...
const SCORE_COLS = {
  Work_at_Height: "T",
  Edge_Protection: "U",
  Falling_Object_Prevention: "V",
  Excavation_Safety: "W",
  Scaffolding_Compliance: "X",
  PTW_Implementation: "Y",
  PTW_Field_Verification: "Z",
  MSRA_Quality: "AA",
  "Lifting/Precast Installation": "AB",
  Traffic_Interface: "AC",
  "Housekeeping/Waste Management": "AD",
  "Welfare Arrangement": "AE",
  Fire_Readiness: "AF",
  Supervision_Subcontractor: "AG",
  Electrical_Tool_Safety: "AH"
};
...
async function generateWorkbook() {
  ...
  try {
    ...
    for (let r = 4; r <= ref.e.r + 1; r++) {
      ...
      const clusterCell = ws["F"+r];
      if (dateCell && clusterCell) {
        const v = XLSX.SSF.format("yyyy-mm-dd", dateCell.v);
        existingRows.push({ date: v, cluster: clusterCell.v });
      }
    }
    ...
    setCell("D"+row, formattedMonth(inspectionDate), "s");
    setCell("E"+row, Number(pkg), "n");
    setCell("F"+row, cluster, "s");
    setCell("G"+row, meta.package === pkg ? "OK" : "CHECK", "s");
    setCell("H"+row, "TAJV", "s");
    setCell("I"+row, meta.pd, "s");
    setCell("J"+row, meta.pm, "s");
    setCell("K"+row, meta.cm, "s");
    setCell("L"+row, meta.hsem, "s");
    setCell("M"+row, meta.se, "s");
    setCell("N"+row, $("#yourName").value || "Javed Iqbal", "s");
    setCell("O"+row, $("#yourRole").value || "KEO", "s");
    setCell("P"+row, $("#areaText").value || `Package ${pkg} - ${cluster} scorecard walk`, "s");
    const workers = $("#workersObserved").value;
    if (workers) setCell("Q"+row, Number(workers), "n");
    setCell("R"+row, mainActivity, "s");
    setCell("S"+row, $("#stopWork").value || "YES", "s");
    ...
    setCell("AI"+row, total, "n");
    setCell("AJ"+row, raw, "n");
    setCell("AK"+row, weightedPct, "n");
    setCell("AL"+row, criticalRedFlag ? "YES" : "NO", "s");
    setCell("AM"+row, award, "s");
    setCell("AN"+row, band, "s");
    setCell("AO"+row, "YES", "s");
    setCell("AP"+row, rootCause, "s");
    setCell("AQ"+row, texts.gaps, "s");
    setCell("AR"+row, texts.immediate, "s");
    setCell("AS"+row, texts.preventive, "s");
    setCell("AT"+row, "", "s");
    setCell("AU"+row, "TAJV", "s");
    const closeout = $("#targetCloseoutDate").value;
    if (closeout) setCell("AV"+row, excelDateSerial(closeout), "n", "yyyy-mm-dd");
    setCell("AW"+row, "", "s");
    const lowNames = lowScoreItems(scores).map(x => x.name).join(", ");
    setCell("AX"+row, weightedPct !== null && weightedPct < 0.6 ? lowNames : "", "s");
    setCell("AY"+row, weightedPct !== null && weightedPct < 0.6 ? lowNames : "", "s");
    setCell("AZ"+row, weightedPct !== null && weightedPct < 0.75 ? lowNames : "", "s");
    setCell("BA"+row, weightedPct !== null && weightedPct < 0.75 ? lowNames : "", "s");
    setCell("BB"+row, weightedPct !== null && weightedPct < 0.75 ? lowNames : "", "s");
    setCell("BC"+row, parseNumericScore(scores["Supervision & Subcontractor Control"]) !== null && parseNumericScore(scores["Supervision & Subcontractor Control"]) >= 3 ? "YES" : "NO", "s");
    setCell("BD"+row, "YES", "s");
    setCell("BE"+row, "", "s");
    setCell("BF"+row, weightedPct !== null && weightedPct < 0.6 ? cluster : "", "s");
